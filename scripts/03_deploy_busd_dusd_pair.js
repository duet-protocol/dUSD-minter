...
const BUSD_Addr = require(`../abis/BUSD.${network.name}.json`)
...
async function main() {
  ...
  const pair = await DuetUSDMinerPair.deploy(BUSD_Addr.address, LinkUSDOracle_Addr.address, DUSD_Addr.address, owner.address);
  await pair.deployed();

  console.log("DuetUSDMinerPair deployed to:", pair.address);
  
  await writeAddr(pair.address, "BUSD-DUSD-Pair", network.name)
